...
dataset_path = '/Users/wenjie/Workspace/data/aclImdb/'
...
# tokenizer_cmd = ['./tokenizer.perl', '-l', 'en', '-q', '-no-escape', '-']
tokenizer_cmd = ['java', 'edu.stanford.nlp.process.PTBTokenizer', '-options',
                 'ptb3Escaping=false', '-preserveLines']
...
def tokenize(sentences):
  ...
  f.write(text.replace('<br />', '<br/>'))
  f.close()
  tokenizer = Popen(tokenizer_cmd + [f.name],
                    stdin=PIPE,
                    stdout=PIPE,
                    stderr=PIPE)
  tok_text, _ = tokenizer.communicate()
  toks = tok_text.split('\n')[:-1]
  os.remove(f.name)
  print 'Done'
